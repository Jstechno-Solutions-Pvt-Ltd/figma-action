    ...
    return queueTasks(Object.values(components).map(component => () => {
      ...
      .then(response => {
        return ensureDir(join(options.outputDir, options.format))
          .then(() =>  console.log(join(options.outputDir, options.format,component.ctype)))
          .then(() =>  ensureDir(join(options.outputDir, options.format,component.ctype)))
          .then(() => writeFile(join(options.outputDir, options.format,component.ctype, component.filename), response.body, (options.format === 'svg' ? 'utf8' : 'binary')))
      })
    }))
  })
  .catch(error => {
